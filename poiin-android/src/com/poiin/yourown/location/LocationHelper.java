package com.poiin.yourown.location;
...
import com.poiin.yourown.Constants;
...
public class LocationHelper {
	...
	public static final GeoPoint GOLDEN_GATE = new GeoPoint(
			(int) (37.49 * LocationHelper.MILLION),
	...
	// parse geoRssPoint into GeoPoint(<georss:point>36.835
	// -121.899</georss:point>)
	public static GeoPoint getGeoPoint(final String geoRssPoint) {
		Log.d(Constants.LOGTAG, LocationHelper.CLASSTAG
				+ " getGeoPoint - geoRssPoint - " + geoRssPoint);
		GeoPoint returnPoint = null;
		String gPoint = geoRssPoint.trim();
		if (gPoint.indexOf(" ") != -1) {
			String latString = gPoint.substring(0, gPoint.indexOf(" "));
			String lonString = gPoint.substring(gPoint.indexOf(" "),
					gPoint.length());
	...
	public static String parsePoint(final double point, final boolean isLat) {
		Log.d(Constants.LOGTAG, LocationHelper.CLASSTAG
				+ " parsePoint - point - " + point + " isLat - " + isLat);
