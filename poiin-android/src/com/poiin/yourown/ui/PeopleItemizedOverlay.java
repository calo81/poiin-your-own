...
public class PeopleItemizedOverlay extends ItemizedOverlay<PersonOverlayItem> {
	...
	protected boolean onTap(int index) {
		...
		Intent i = new Intent(Intent.ACTION_VIEW,Uri.parse("poiin://poiin.yourown/"+item.getPerson().getId()+"?name="+item.getPerson().getName()+"&poiinText="+item.getPerson().getPoiinText()+"&categories="+item.getPerson().getSelectedCategories().toString()));
